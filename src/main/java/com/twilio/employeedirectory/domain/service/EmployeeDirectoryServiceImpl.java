...
public class EmployeeDirectoryServiceImpl implements EmployeeDirectoryService {
     ...
     * @return an {@link Optional} with the {@link Employee} correspondant to the <code>optionIndex</code>
     */
        ...
        return availableOptions
                .map(
                        options -> options.stream().filter(pair -> pair.getName().equals(optionIndex))
                                .findFirst()).map(this::getEmployeeFromOption).get();
    }

    /**
     * Given some option returns the correspondant {@link Employee}
     *
     * @param choosenOption an {@link Optional} of {@link NameValuePair} linking the chosen option index with the wanted
     *                      {@link Employee#id}
    ...
    protected Optional<Employee> getEmployeeFromOption(Optional<NameValuePair> choosenOption) {
        return Utils.getOptionalLong(choosenOption.get().getValue()).map(repository::findEmployeeById).get();
    }
}
